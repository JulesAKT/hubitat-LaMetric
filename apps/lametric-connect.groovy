 ...
 *  Author: Mykola Kirichuk (with Hubitat fixup by 'AaronW' and Jules Taplin)
 *
...
definition(
    ...
    author: "Mykola Kirichuk (with Hubitat fixup by 'AaronW' & Jules Taplin)",
...
def getVerifiedDevices() {
    ...
    //getDevices().findAll{ it?.value?.name != null }
...
def deviceDiscovery()
{
    ...
    if (1)
    {
        ...
        return dynamicPage(name:"deviceDiscovery", title:"Discovery Started!", nextPage:"deviceDiscovery", refreshInterval:refreshInterval, install:true, uninstall: true) {
...
def discoveryCallBackHandler(response)
{
	log.trace "Entered discoveryCallBackHandler()"
	            //log.trace "lan event ${lanEvent}";
	log.debug "Response = ${response.body}"
            def parsedJsonBody;
            try {
                parsedJsonBody = parseJson(response.body);
            } catch (e)
            {
    ...
    devlist.each { dni ->
        ...
        if (newDevice)
        {
            ...
            if(!d) {
				...
				log.trace "NS: "+getNameSpace()+" Device Name: "+getDeviceName()+" DNI: "+newDevice.dni+" Hub: "+newDevice.hub
...
private getVendorTokenPath()		{ "${apiUrl}/api/v2/oauth2/token" }
private getVendorAuthPath()			{ "/api/v2/oauth2/authorize/" }
private getSmartThingsClientId() 		{ settings.clientId }
private getSmartThingsClientSecret() 	{ settings.clientSecret }
//def getSmartThingsClientId() 		{ "f3e4e2d7-0abd-6f62-86e5-934de27b9a94" }
//def getSmartThingsClientSecret() 	{ "JDEkcXYuV1B3cjEkUHZKSzdWQTBXZnBQMk4wd2h6MkVGMQ==" }
...
private getApiUrl()	        		{ "https://developer.lametric.com" }
private getAuthScope() 					{ [ "basic", "devices_read" ] }
private	getVendorName()				{ "LaMetric" }
private getApiUserMeDevicesList()		{ "/api/v2/users/me/devices" }
...
def authPage() {
    ...
    if (!oauthTokenProvided) {
        return dynamicPage(name: "auth", title: "Login", nextPage: "", uninstall:uninstallAllowed) {
			section("Enter Client API Details...") {
					paragraph "Sign up at https://developer.lametric.com"
            ...
            section(){
				...
				href url:oauthInitUrl(), external:true, required:false, title:"Connect to ${vendorName}", description:description
            }
        }
    } else {
        subscribeNetworkEvents()
...
def callback() {
    ...
    if (oauthState == state.oauthInitState){
		...
		tokenUrl = getVendorTokenPath()
		
		//log.trace "TOKEN Params: "+ toQueryString(tokenParams)
        log.trace "Token URL: "+ tokenUrl + toQueryString(tokenParams)
        try {
...
def oauthInitUrl() {
    ...
    log.debug "${apiUrl}/api/v2/oauth2/authorize?${toQueryString(oauthParams)}"
...
def success() {
    def message = """
<p>Your LaMetric Account is now connected to Hubitat!</p>
...
def notificationCallbackMethod(response,data)
{
	log.debug "!!Notification callback entered - status: ${response.status} (${response.data})" 
...
def sendNotificationMessageToDevice(dni, data)
{
    ...
    if (localIp && apiKey)
    {
		log.debug "send notification message to device ${localIp}:8080${localApiSendNotificationPath} ${data} - key: ${apiKey}"
 ...
 */
		log.debug "Auth: ${localApiUser}:${apiKey}"
		asynchttpPost('notificationCallbackMethod', [ 
			//path: localApiSendNotificationPath,
			uri: "http://${localIp}:8080/${localApiSendNotificationPath}",
			//uri: "http://192.168.2.86:8080/${localApiSendNotificationPath}",
            body: data,
...
{
    log.debug "send something - with a callback"
	log.debug "LocalApiIndexPath = ${localApiIndexPath}, IP = ${localIp}, LocalAPIUser = ${localApiUser}"
    if (localIp && apiKey)
    {
    	def hubCommand = new hubitat.device.HubAction([
 		...
 		[callback: discoveryCallBackHandler])
